define(function(require, exports, module) {
	...
	function loadData() {
		...
		$(grid_selector).jqGrid({
			...
			//viewrecords: true,  //
			jsonReader: {root: 'data.pageData', page: 'data.pageNo', total: 'data.pageCnt'}, //数据结构映射
			colNames: ['字典类型编码', '字典类型名称', '操作'],
			colModel: [{
				name: 'type',
				index: 'type',
				//width: 60,
			...
			gridComplete: function(){
				...
				for(var i=0;i < ids.length;i++){
					var bts = ''
					var cl = ids[i];
					var d = $(this).jqGrid("getRowData",cl);
					...
					$(grid_selector).jqGrid('setRowData',ids[i],{type:'<a href="/dataDictionary/dropdownlist?type=' + d.type + '">' + d.type + '</a>'});
